...
function Product() {
  ...
  if (!product) return <h2 style={{ textAlign: "center" }}>Product nahi mila!</h2>;
  ...
  return (
    <div style={{ padding: "30px", maxWidth: "600px", margin: "0 auto" }}>
      ...
      <div style={{ marginTop: "20px", border: "1px solid #ddd", borderRadius: "12px", padding: "25px" }}>
        ...
        <h3 style={{ color: "#555" }}>{product.price}</h3>
        <p style={{ fontSize: "16px", color: "#444", lineHeight: "1.6" }}>{product.description}</p>
        {product.price && (
          <p style={{ fontSize: "22px", fontWeight: "bold", color: "#e44" }}>${product.price}</p>
        )}
        {product.description && <p>⭐ Rank: {product.description}</p>}
        {product.age && <p>Age: {product.image}</p>}
      </div>
    </div>
  );
}
