...
function Product() {
  ...
  if (!item) return <h2 style={{ textAlign: "center" }}>Product nahi mila!</h2>;
  ...
  return (
    <div style={{ padding: "30px", maxWidth: "600px", margin: "0 auto" }}>
      ...
      <div style={{ marginTop: "20px", border: "1px solid #ddd", borderRadius: "12px", padding: "25px" }}>
        ...
        <h1 style={{ marginTop: "15px" }}>{item.name}</h1>
        <h3 style={{ color: "#555" }}>{item.price}</h3>
        <p style={{ fontSize: "16px", color: "#444", lineHeight: "1.6" }}>{product.description}</p>
        {item.price && (
          <p style={{ fontSize: "22px", fontWeight: "bold", color: "#e44" }}>${item.price}</p>
        )}
        {item.description && <p>⭐ Rank: {item.description}</p>}
        {item.age && <p>Age: {item.image}</p>}
      </div>
    </div>
  );
}
